...
export default function PixelWave({ waveRef, canvasRef }) {
  useEffect(() => {
    ...
    const draw = () => {
      if (!wrapper || !canvas || !context) return;
      if (wrapper.offsetWidth !== width || wrapper.offsetHeight !== height) resize();
      ...
      const spacing = Math.max(5, Math.min(8, (width + height) / 38));
      ...
      for (let row = 0; row < rows; row += 1) {
        ...
        for (let col = 0; col < cols; col += 1) {
          ...
          const noise =
            ...
            6 +
            0.5;
          const threshold = 0.1 + noise * 0.72;
          const pixelReveal = Math.max(0, Math.min(1, (reveal - threshold) * 4.2));
          if (pixelReveal <= 0.02) continue;

          const wave =
            Math.sin(x * 0.02 + phase + row * 0.25) * (8 + intensity * 10) +
            Math.sin(x * 0.035 - phase * 0.9 + row * 0.15) * (4 + intensity * 8);
          const y = gridY + wave * waveBlend * (0.35 + intensity * 0.65);
      ...
      for (let row = 0; row < rows; row += 1) {
        ...
        for (let col = 0; col < cols; col += 1) {
          ...
          const wave =
            ...
            Math.sin(x * 0.035 - phase * 0.9 + row * 0.15) * (4 + intensity * 8);
          const y = gridY + wave * waveBlend * (0.35 + intensity * 0.65);
          const variation = (Math.sin(x * 0.07 + row * 0.6 + phase * 0.4) + 1) / 2;
          const size = spacing * (0.62 + variation * 0.58 * density) * (0.76 + pixelReveal * 0.34);
